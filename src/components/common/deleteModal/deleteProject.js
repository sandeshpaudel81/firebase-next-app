import { deleteProject, deleteProjectReset, fetchProjects } from '@/redux/slices/projectSlice'
import { useRouter } from 'next/router'
import React, { useEffect, useState } from 'react'
import toast from 'react-hot-toast'
import { useDispatch, useSelector } from 'react-redux'

const DeleteProjectModal = ({setShowModal, id}) => {
    const dispatch = useDispatch()
    const router = useRouter()
    const {success:deleteProjectSuccess, error:deleteProjectError} = useSelector(state => state.project.deleteProject)
    ...
    return (
        <div className='fixed top-0 left-0 h-screen w-screen z-50 p-5'>
            ...
            <div className='relative top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gray-700 w-[500px] p-5 text-center rounded-lg'>
                ...
                <h3 className="mb-5 text-lg font-normal text-gray-400 dark:text-gray-400">Are you sure you want to delete this project?</h3>
